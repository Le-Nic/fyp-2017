...
def genDOCX(colr, fileName, merged, isPDF=False):
	...
	for vuln in warehouse.vulnList:
		...
		if vuln not in processedIndexes:
			processedIndexes.update({vuln: []})

		# ----- Vulnerabilities -----
		for index, row in vuln.data.iterrows():
			# skip vulnerability if merged

			if index not in processedIndexes[vuln]:

				currentIndex = [{vuln: index}]

				processedIndexes[vuln].append(index)
				...
				if row['name'] is not None and row['name'] != '':
					...
					for vuln2 in warehouse.vulnList:

						if vuln.scanner == vuln2.scanner:

							for index2, row2 in vuln2.data.loc[vuln2.data['name'] == row['name']].iterrows():

								run = paragraph.add_run('\t'+vuln2.ip)
								run.font.size = Pt(11)

								currentIndex.append({vuln2: index})

								if vuln2 not in processedIndexes:
									processedIndexes.update({vuln2: [index2]})
								else:
									processedIndexes[vuln2].append(index2)
